...
const MAP_DB_TO_UI = {
  ...
  comp: { apoyo_economico: "Apoyo económico", "Apoyo económico": "Apoyo económico", sin_apoyo: "Sin apoyo", "Sin apoyo": "Sin apoyo" },
};
...
const IconBookmark = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" {...props} aria-hidden>
    <path fill="currentColor" d="M6 2h12a1 1 0 0 1 1 1v19l-7-4-7 4V3a1 1 0 0 1 1-1Z" />
  </svg>
);
const IconBan = (props) => (
  <svg width="18" height="18" viewBox="0 0 24 24" {...props} aria-hidden>
    <path fill="currentColor" d="M12 2a10 10 0 1 0 10 10A10.011 10.011 0 0 0 12 2Zm0 2a8 8 0 0 1 5.292 13.938L6.062 6.708A7.963 7.963 0 0 1 12 4Zm-8 8a8 8 0 0 1 13.938 5.292L6.708 6.062A7.963 7.963 0 0 0 4 12Z"/>
  </svg>
);
...
export default function EstudiantesPage() {
  ...
  // Estado de usuario/favoritos/ocultas
  const [userId, setUserId] = useState(null);
  const [favIds, setFavIds] = useState([]);      // uuid[]
  const [hiddenIds, setHiddenIds] = useState([]); // uuid[]

  // Carga usuario + favoritos/ocultas
  useEffect(() => {
    let ignore = false;
    const boot = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return; // el middleware ya protege, pero por si acaso
      if (ignore) return;
      setUserId(user.id);
      ...
      // ocultas
      const { data: hidData } = await supabase
        .from("vacancy_hidden")
        .select("vacancy_id")
        .eq("student_id", user.id)
        .limit(500);
  ...
  // Carga de vacantes (incluye excluir ocultas)
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (q) {
        ...
        const likeStar = `*${safe}*`;
        const parts = [
          `title.ilike.${likeStar}`,
          `location_text.ilike.${likeStar}`,
        ];
      ...
      // 6) excluir ocultas (si hay)
      if (hiddenIds.length) {
        // PostgREST usa formato 'in.(a,b,c)'; con not: .not('id','in',`(${...})`)
    ...
    fetchData();
  }, [q, loc, filters, page, hiddenIds]); // <- si ocultas cambia, refrescamos

  /* --------- acciones: favoritos / ocultar ---------- */
  const toggleFavorite = async (vacancyId) => {
    if (!userId) return;
    const isFav = favIds.includes(vacancyId);
    if (isFav) {
      ...
      if (!error) setFavIds((s) => s.filter((id) => id !== vacancyId));
    } else {
      const { error } = await supabase
        .from("vacancy_favorites")
        .insert({ student_id: userId, vacancy_id: vacancyId });
      if (!error) setFavIds((s) => [...s, vacancyId]);
    }
  };

  const toggleHidden = async (vacancyId) => {
    if (!userId) return;
    const isHidden = hiddenIds.includes(vacancyId);
    if (isHidden) {
      const { error } = await supabase
        .from("vacancy_hidden")
        .delete()
        .eq("student_id", userId)
        .eq("vacancy_id", vacancyId);
      if (!error) setHiddenIds((s) => s.filter((id) => id !== vacancyId));
    } else {
      const { error } = await supabase
        .from("vacancy_hidden")
        .insert({ student_id: userId, vacancy_id: vacancyId });
      if (!error) setHiddenIds((s) => [...s, vacancyId]);
    }
  };
    ...
    <>
      ...
      <main className="jobs-wrap">
        ...
        <div className="jobs-filters">
          <Pill label="Modalidad" value={filters.modalidad}
                onChange={(v) => { setPage(0); setFilters((s) => ({ ...s, modalidad: v })); }}
                options={MODALIDADES}/>
          <Pill label="Compensación" value={filters.comp}
                onChange={(v) => { setPage(0); setFilters((s) => ({ ...s, comp: v })); }}
                options={COMPENSACIONES}/>
          <Pill label="Idioma" value={filters.idioma}
        ...
        <section className="jobs-grid">
          ...
          <aside className="jobs-listing">
            {loading && <>
              {Array.from({ length: 6 }).map((_, i) => <div className="jobs-card sk" key={i}/>)}
            </>}
            ...
            {!loading && selected && (
              <div className="jobs-detail-inner">
                <header className="jobs-detail-head">
                  ...
                  <div className="jobs-card-actions">
                    <IconBtn
                      title={favIds.includes(selected.id) ? "Quitar de favoritos" : "Agregar a favoritos"}
                      active={favIds.includes(selected.id)}
                    ...
                    <IconBtn
                      title={hiddenIds.includes(selected.id) ? "Mostrar esta vacante" : "Ocultar esta vacante"}
                      active={hiddenIds.includes(selected.id)}
